...
const DataBooks = () => {
  ...
  const columns = [
    ...
    },
    {
      field: "updatedAt",
      header: "Updated At",
      body: (rowData) => rowData.updatedAt && formatDate(rowData.updatedAt),
    },
    {
      field: "deletedAt",
      header: "Deleted At",
      body: (rowData) => rowData.deletedAt && formatDate(rowData.deletedAt),
    },
  ...
  const [query, setQuery] = useState({
    ...
    limit: 10,
    // sort: "createdAt:asc"
    // fields: ['title', 'author.name', 'year', 'ISBN', 'stock']
    // filters: {
      // updatedAt: new Date(),
      // isAdmin: false
    // },
 ...
 const bookService = useAllBooks(query);
  ...
  return (
    <div className="card">
      ...
      <Button
        label="Add Book"
        icon="pi pi-plus"
        onClick={() => navigate("/app/books/create")}
      />
            ...
            <div className="flex ">
              {!rowData.deletedAt && (
                <>
                  <Button
                    label="Edit"
                    onClick={() => navigate(`/app/books/edit/${rowData.id}`)}
                    icon="pi pi-pencil"
                    size="small"
...
function AddBook({
  ...
  async function handleCreateBook(e) {
    ...
    console.log(
      ...
      genres.find((g) => g.name === genreId)?.id,
      safePdf,
      stock
    );
    try {
      if (id) {
        await updateBook.mutateAsync({
          ...
          payload: {
            ...
            ISBN,
            cover,
            categoryId: categories.find((c) => c.name === categoryId)?.id,
            genreId: genres.find((g) => g.name === genreId)?.id,
            pdf: safePdf,
            stock,
            updatedById: user.id,
          },
