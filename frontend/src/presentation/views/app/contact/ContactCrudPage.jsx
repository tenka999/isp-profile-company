...
import { useContactApi } from "@/presentation/logics/app/useContactApi";
...
const ContactCrudPage = () => {
  ...
  const contactStatuses = [
    ...
    { label: "Follow Up", value: "FOLLOWED_UP" },
  ];

  const { data, isPending, isError, error, refetch } = useAllContact();
  ...
  const newContacts =
    contacts?.filter((val) => val.status === "NEW").length || 0;
  const readContacts =
  ...
  const newAnim = useCountUp(newContacts);
    ...
    if (contact.nama.trim() && contact.email.trim() && contact.pesan.trim()) {
      ...
      try {
        if (contact.id) {
          ...
          const result = await createContact.mutateAsync(_contact);
  ...
  const deleteContactItem = async () => {
    try {
      ...
      const result = await deleteContact.mutateAsync({ id: contact.id });
  ...
  const deleteSelectedContacts = async () => {
    try {
      ...
      const response = await deleteContacts.mutateAsync({
        ids: ids,
      });
      setContacts(_contacts);
      setDataContacts(_contacts);
  ...
  const statusBodyTemplate = (rowData) => {
    const map = {
      NEW: { label: "Belum Dibaca", severity: "danger" },
      READ: { label: "Sudah Dibaca", severity: "info" },
      FOLLOWED_UP: { label: "Sudah Dibalas", severity: "success" },
    };
    ...
    <div className="grid">
      ...
      <div className="col-2 md:col-0">
        ...
        >
          ...
          <i className="pi pi-reply text-3xl text-green-500" />
        </div>
      </div>
